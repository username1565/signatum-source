...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("signatum"))
        return false;
...
{
    // Convert signatum:// to signatum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("signatum://"))
    {
        uri.replace(0, 12, "signatum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Signatum.lnk";
}
...
{
    return GetAutostartDir() / "signatum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Signatum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  signatum-qt [" + tr("command-line options") + "]                     " + "\n";
